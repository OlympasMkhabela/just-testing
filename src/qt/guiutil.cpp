...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a XGA address (e.g. %1)").arg("Xfomf2vcNyX6j34gQQEns6cQW6uQx87j2b"));
#endif
...
{
    // return if URI is not valid or is no lega: URI
...
{
    // Convert lega:// to lega:
    //
    //    Cannot handle this later, because lega:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open lega.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Lega.lnk";
}
...
{
    return GetAutostartDir() / "lega.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a lega.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Lega\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lega app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add lega app to startup item list
